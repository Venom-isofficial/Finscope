    ...
    <div className="flex flex-col pb-32 pt-6 px-4">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 className="font-display text-2xl font-bold text-brand-text-primary tracking-tight">FinScope Editorial</h1>
        </div>
        <button onClick={loadNews} className="p-3.5 rounded-[18px] bg-brand-card border border-brand-text-primary/5 text-brand-text-secondary hover:text-brand-text-primary hover:border-brand-accent/50 transition-all shadow-lg cursor-pointer">
          <RefreshCw className="w-4 h-4" />
        </button>
      </div>

      {/* Primary Toggles */}
      <div className="grid grid-cols-2 gap-2 bg-brand-card rounded-[18px] p-1.5 border border-brand-text-primary/5 mb-8 shadow-md">
        <button
          onClick={() => setActiveTab("latest")}
          className={`py-3 rounded-[14px] text-xs font-bold tracking-widest uppercase transition-all cursor-pointer ${
        ...
        <button
          ...
          className={`py-3 rounded-[14px] text-xs font-bold tracking-widest uppercase transition-all flex items-center justify-center gap-2 cursor-pointer ${
        ...
        {activeTab === "latest" ? (
          ...
          >
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-[10px] font-bold tracking-widest uppercase whitespace-nowrap transition-all cursor-pointer ${
                    selectedCategory === cat
                      ? "bg-brand-accent text-brand-primary shadow-md shadow-brand-accent/10"
                      : "bg-brand-card/75 text-brand-text-secondary border border-brand-text-primary/5 hover:text-brand-text-primary hover:border-brand-accent/30"
                  }`}
                >
                  {cat}
                </button>
            ...
            {filteredNews.length > 0 && query === "" && selectedCategory === "All" && (
              <div
                ...
                className="relative h-64 rounded-2xl overflow-hidden border border-brand-text-primary/5 cursor-pointer shadow-lg shadow-black/5 shadow-black/10 group"
              >
                ...
                <div className="absolute bottom-5 left-5 right-5">
                  <span className="bg-brand-accent text-brand-primary text-[9px] font-bold tracking-widest uppercase px-2.5 py-1 rounded-[6px] mb-2.5 inline-block shadow-lg">
                    {filteredNews[0].category}
                  </span>
                  <h2 className="text-base font-bold text-brand-text-primary line-clamp-2 leading-tight group-hover:text-brand-accent transition-colors duration-200 ease-out">
                ...
                {filteredNews.slice(query === "" && selectedCategory === "All" ? 1 : 0).map((item) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex gap-4 p-4 rounded-2xl bg-gradient-to-r from-brand-card/80 to-brand-secondary/80 border border-brand-text-primary/5 hover:border-brand-accent/40 hover:shadow-lg shadow-black/5 shadow-black/10 hover:translate-x-1 transition-all cursor-pointer shadow-lg"
                    >
                      <div className="flex-1 flex flex-col justify-between">
                        <div>
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="text-[9px] font-bold font-mono text-brand-accent bg-brand-accent/10 px-2 py-0.5 rounded uppercase">{item.category}</span>
                            <span className="text-[9px] font-mono text-brand-text-secondary font-semibold">{item.source}</span>
                          </div>
                          <h3 className="text-xs font-bold text-brand-text-primary leading-snug line-clamp-2 hover:text-brand-accent transition-colors duration-200 ease-out">
                            {item.headline}
                          </h3>
                        </div>
                        
                        <div className="flex items-center justify-between mt-3 pt-2.5 border-t border-brand-text-primary/5">
                          ...
                          <button
                            ...
                            className={`p-1.5 rounded-lg border transition-colors ${
                              isSaved ? "text-brand-accent bg-brand-accent/15 border-brand-accent/30" : "text-brand-text-secondary border-brand-text-primary/5 hover:bg-brand-text-primary/5"
                            }`}
                          >
          ...
          >
            {savedArticles.length > 0 ? (
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      <div className="flex items-center justify-between mb-1.5 text-[9px] font-mono uppercase">
                        <span className="text-brand-accent font-bold bg-brand-accent/10 px-2 py-0.5 rounded">{article.category}</span>
                    ...
                    <div className="flex items-center justify-between mt-3 pt-2.5 border-t border-brand-text-primary/5">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-red-500/10 text-red-400 border border-red-500/15 hover:bg-red-500/20 transition-all duration-200 ease-out"
              ...
              <div className="text-center py-16 bg-brand-card/10 rounded-2xl border border-dashed border-brand-text-primary/5 p-6">
