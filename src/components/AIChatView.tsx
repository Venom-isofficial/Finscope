    ...
    <div className="flex flex-col h-[calc(100vh-140px)] pt-6">
      ...
      <div className="flex items-center justify-between mb-5 px-4">
        <div className="flex items-center gap-2.5">
          <div className="w-10 h-10 rounded-2xl bg-[var(--accent)]/15 border border-[var(--accent)]/20 flex items-center justify-center text-[var(--accent)] shadow-[0_0_20px_rgba(182,255,90,0.15)]">
        ...
        <button
          ...
          className="p-3 rounded-[14px] bg-[var(--bg-card)] border border-white/10 text-[var(--text-secondary)] hover:text-[var(--chart-red)] hover:border-[var(--chart-red)]/40 transition-all shadow-md"
      ...
      <div className="mx-4 mb-5 p-4 bg-gradient-to-br from-[var(--bg-card)]/70 to-[var(--bg-secondary)]/70 border border-white/10 rounded-[20px] flex items-start gap-3 text-[10px] text-[var(--text-secondary)] leading-relaxed shadow-lg">
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-4 pb-6">
            ...
            >
              <div
                className={`max-w-[85%] p-4 rounded-[24px] text-xs leading-relaxed border shadow-md ${
                  isAI
                    ? "bg-gradient-to-br from-[var(--bg-card)]/90 to-[var(--bg-secondary)]/90 text-white rounded-tl-none border-white/10"
                    : "bg-[var(--accent)] text-[var(--bg-primary)] rounded-tr-none border-[var(--accent)]/20 font-bold"
                }`}
              >
                {isAI && (
                  <div className="flex items-center gap-1.5 text-[9px] font-mono font-bold text-[var(--accent)] uppercase mb-2 pb-2 border-b border-white/10">
        ...
        {sending && (
          <div className="flex justify-start">
            <div className="bg-gradient-to-br from-[var(--bg-card)]/90 to-[var(--bg-secondary)]/90 border border-white/10 p-4 rounded-[24px] rounded-tl-none flex items-center gap-2 shadow-md">
          ...
          <div className="flex flex-col gap-2.5">
            {suggestions.map((sug) => (
              <button
                ...
                className="text-left py-3 px-4 rounded-[16px] bg-[var(--bg-card)]/40 border border-white/10 text-[11px] text-[var(--text-secondary)] hover:text-white hover:border-[var(--accent)]/40 transition-all flex items-center justify-between group shadow-sm"
              >
      ...
      {/* Footer input form */}
      <form onSubmit={handleSubmit} className="p-4 bg-[var(--bg-primary)]/95 backdrop-blur-md border-t border-white/10">
        <div className="relative flex items-center max-w-2xl mx-auto w-full">
          <input
            ...
            disabled={sending}
            className="w-full pl-5 pr-14 py-4 rounded-[20px] bg-[var(--bg-card)]/80 border border-white/10 focus:border-[var(--accent)]/50 focus:bg-[var(--bg-card)] outline-none text-white text-xs transition-all shadow-md placeholder-[var(--text-secondary)]/60 font-medium"
          />
          <button
            type="submit"
            disabled={!message.trim() || sending}
            className="absolute right-2.5 p-2.5 rounded-[12px] bg-[var(--accent)] text-[var(--bg-primary)] hover:bg-[var(--accent-highlight)] transition-colors disabled:opacity-40 disabled:cursor-not-allowed shadow-md"
          >
