    ...
    <div className="flex flex-col pb-32 pt-6 px-4">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 className="font-display text-2xl font-bold text-brand-text-primary tracking-tight">Assets Explorer</h1>
        </div>
        <button onClick={loadMarketData} className="p-3.5 rounded-[18px] bg-brand-card border border-brand-text-primary/5 text-brand-text-secondary hover:text-brand-text-primary hover:border-brand-accent/50 transition-all shadow-lg">
          <RefreshCw className="w-4 h-4" />
        </button>
      </div>

      {/* Internal Tab Filter Selector */}
      <div className="flex gap-2 overflow-x-auto pb-4 border-b border-brand-text-primary/5 mb-6 no-scrollbar">
        {[
          ...
          <button
            ...
            className={`px-4.5 py-3 rounded-[14px] text-xs font-bold tracking-widest uppercase whitespace-nowrap transition-all ${
              activeTab === tab.id
                ? "bg-brand-accent text-brand-primary shadow-lg shadow-brand-accent/10"
                : "bg-brand-card/70 border border-brand-text-primary/5 text-brand-text-secondary hover:text-brand-text-primary hover:border-white/20"
            }`}
          >
            {tab.label}
          </button>
      ...
      {activeTab !== "watchlist" && (
        <div className="relative mb-6">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-3.5 rounded-[18px] bg-brand-card/80 border border-brand-text-primary/5 focus:border-brand-accent/50 focus:bg-brand-card outline-none text-brand-text-primary text-xs transition-all shadow-md placeholder-brand-text-secondary/60"
          />
        </div>
      )}
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="h-20 bg-brand-text-primary/5 animate-pulse rounded-2xl" />
          ))}
        </div>
      ) : (
        
          <div
            key={activeTab} className="space-y-3"
          >
            {/* STOCKS LIST */}
              ...
              .map((st) => {
                ...
                return (
                  <div
                    ...
                    className="flex items-center justify-between p-4 rounded-[20px] bg-gradient-to-r from-brand-card/80 to-brand-secondary/80 border border-brand-text-primary/5 hover:border-brand-accent/40 shadow-lg hover:translate-x-1 transition-all cursor-pointer"
                  >
                    <div className="flex items-center gap-3">
                      <button
                        onClick={(e) => handleToggleWatchlist(e, st.symbol, st.symbol + " Inc.", "stock")}
                        className={`p-2.5 rounded-[12px] border transition-all ${
                          isWatch ? "bg-brand-accent/15 border-brand-accent/30 text-brand-accent" : "bg-brand-card border-brand-text-primary/5 text-brand-text-secondary"
                        }`}
                      >
                    ...
                    <div className="text-right">
                      <p className="text-xs font-bold font-mono text-brand-text-primary">${st.price.toFixed(2)}</p>
              ...
              .map((cr) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="text-xs font-bold font-mono text-brand-text-primary">${cr.price.toLocaleString()}</p>
                      <span className={`text-[10px] font-mono font-bold flex items-center justify-end gap-0.5 ${positive ? "text-brand-chart-green" : "text-brand-chart-red"}`}>
                        {positive ? <ArrowUpRight className="w-3 h-3" /> : <ArrowDownRight className="w-3 h-3" />}
              ...
              .map((et) => {
                ...
                return (
                  <div
                    ...
                    className="p-4 rounded-[20px] bg-gradient-to-r from-brand-card/80 to-brand-secondary/80 border border-brand-text-primary/5 hover:border-brand-accent/40 shadow-lg hover:translate-x-1 transition-all cursor-pointer"
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="text-xs font-bold font-mono bg-brand-accent/15 border border-brand-accent/25 text-brand-accent px-2.5 py-1 rounded-[8px]">
                            {et.symbol}
                          </span>
                          <span className="text-xs text-brand-text-primary font-bold ml-2">{et.name}</span>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-xs font-bold font-mono text-brand-text-primary">${et.price.toFixed(2)}</p>
                    ...
                    <div className="flex items-center gap-3 pt-3 border-t border-brand-text-primary/5 text-[9px] font-mono text-brand-text-secondary font-bold">
                      <span className="flex items-center gap-1.5 bg-brand-primary/60 px-2 py-1 rounded-[6px] border border-brand-text-primary/5">
                        <Award className="w-3.5 h-3.5 text-brand-accent" />
                        Risk: {et.risk}
                      </span>
                      <span>•</span>
                      <span className="bg-brand-primary/60 px-2 py-1 rounded-[6px] border border-brand-text-primary/5">Ratio: {et.expenseRatio}%</span>
                      <span>•</span>
            ...
            {activeTab === "watchlist" && (
              watchlist.length > 0 ? (
                watchlist.map((item) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex items-center justify-between p-4 rounded-[20px] bg-gradient-to-r from-brand-card/80 to-brand-secondary/80 border border-brand-text-primary/5 hover:border-brand-accent/40 shadow-lg hover:translate-x-1 transition-all cursor-pointer relative overflow-hidden"
                    >
                      {/* Swipe / Delete styling details */}
                      <div className="flex items-center gap-3">
                        <button
                          onClick={(e) => { e.stopPropagation(); removeFromWatchlist(item.symbol); }}
                          className="p-2.5 rounded-[12px] bg-red-500/10 text-red-400 border border-red-500/15 hover:bg-red-500/20 transition-all duration-200 ease-out"
                        >
                          <Star className="w-3.5 h-3.5 fill-current" />
                        </button>
                        <div>
                      ...
                      <div className="text-right">
                        <p className="text-xs font-bold font-mono text-brand-text-primary">${priceVal.toLocaleString()}</p>
                        <p className={`text-[10px] font-mono font-bold flex items-center justify-end gap-0.5 ${positive ? "text-brand-chart-green" : "text-brand-chart-red"}`}>
                          {positive ? <ArrowUpRight className="w-3 h-3" /> : <ArrowDownRight className="w-3 h-3" />}
                ...
                <div className="text-center py-12 bg-gradient-to-br from-brand-card/40 to-brand-secondary/40 rounded-2xl border border-dashed border-brand-text-primary/15 p-6 shadow-md">
                  <Star className="w-8 h-8 text-brand-accent/60 mx-auto mb-3" />
                  <p className="text-sm text-brand-text-primary font-bold">Your Watchlist is empty</p>
