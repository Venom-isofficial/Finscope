...
export const NotificationsView: React.FC<NotificationsViewProps> = ({ onBack }) => {
  ...
  const getNotifIcon = (type: string) => {
    if (type === "price_alert") return <TrendingUp className="w-4 h-4 text-[var(--accent)]" />;
  ...
  return (
    <div className="flex flex-col pb-32 pt-6 px-4 max-w-lg mx-auto w-full">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <span className="text-[10px] font-mono tracking-widest text-[var(--text-secondary)] uppercase font-semibold block mb-0.5">CHRONO TIMELINE</span>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-4 py-2.5 rounded-[14px] bg-[var(--bg-card)] border border-white/10 text-[10px] text-white hover:text-[var(--accent)] hover:border-[var(--accent)]/40 font-black flex items-center gap-1.5 transition-all shadow-md"
          >
          ...
          <button
            onClick={onBack}
            className="px-4 py-2.5 rounded-[14px] bg-[var(--bg-card)] border border-white/10 text-[10px] text-[var(--text-secondary)] hover:text-white hover:border-white/20 font-black transition-all shadow-md"
          >
            BACK
          </button>
        </div>
      ...
      <div className="space-y-6">
        {categories.map((cat) => {
          ...
          return (
            <div key={cat} className="space-y-3">
              <h3 className="text-xs font-mono tracking-widest text-[var(--text-secondary)] uppercase flex items-center gap-1.5 border-b border-white/10 pb-2.5 font-bold">
                ...
                {matching.map((n) => (
                  <div
                    ...
                    className={`p-4 rounded-[24px] border transition-all flex gap-3.5 relative overflow-hidden shadow-lg ${
                      n.isRead
                        ? "bg-[var(--bg-card)]/40 border-white/10"
                        : "bg-gradient-to-br from-[var(--bg-card)]/85 to-[var(--bg-secondary)]/85 border-[var(--accent)]/30 shadow-[0_0_20px_rgba(182,255,90,0.04)]"
                    }`}
                  >
                    ...
                    <div className="w-9 h-9 rounded-[10px] bg-[var(--bg-primary)]/80 flex items-center justify-center shrink-0 border border-white/10 shadow-sm">
                      {getNotifIcon(n.type)}
                    </div>
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[11px] text-[var(--text-secondary)] font-medium leading-relaxed">{n.message}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {notifications.length === 0 && (
          <div className="text-center py-12 bg-gradient-to-br from-[var(--bg-card)]/40 to-[var(--bg-secondary)]/40 rounded-[24px] border border-dashed border-white/15 p-6 shadow-md">
            <Bell className="w-8 h-8 text-[var(--accent)]/60 mx-auto mb-3" />
