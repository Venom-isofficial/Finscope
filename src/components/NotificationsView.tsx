    ...
    <div className="flex flex-col pb-32 pt-6 px-4 max-w-lg mx-auto w-full">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 className="font-display text-2xl font-bold text-brand-text-primary tracking-tight">Market Alerts</h1>
        </div>
        
        <div className="flex gap-2">
          <button
            onClick={markAllNotificationsRead}
            className="px-4 py-2.5 rounded-[14px] bg-brand-card border border-brand-text-primary/5 text-[10px] text-brand-text-primary hover:text-brand-accent hover:border-brand-accent/40 font-bold flex items-center gap-1.5 transition-all shadow-md"
          >
          ...
          <button
            onClick={onBack}
            className="px-4 py-2.5 rounded-[14px] bg-brand-card border border-brand-text-primary/5 text-[10px] text-brand-text-secondary hover:text-brand-text-primary hover:border-white/20 font-bold transition-all shadow-md"
          >
            BACK
          </button>
        </div>
      ...
      <div className="space-y-6">
        {categories.map((cat) => {
          ...
          return (
            <div key={cat} className="space-y-3">
              <h3 className="text-xs font-mono tracking-widest text-brand-text-secondary uppercase flex items-center gap-1.5 border-b border-brand-text-primary/5 pb-2.5 font-bold">
              ...
              <div className="space-y-3">
                {matching.map((n) => (
                  <div
                    key={n.id}
                    className={`p-4 rounded-2xl border transition-all flex gap-3.5 relative overflow-hidden shadow-lg ${
                      n.isRead
                        ? "bg-brand-card/40 border-brand-text-primary/5"
                  ...
                  >
                    ...
                    <div className="w-9 h-9 rounded-[10px] bg-brand-primary/80 flex items-center justify-center shrink-0 border border-brand-text-primary/5 shadow-sm">
                      {getNotifIcon(n.type)}
                    </div>

                    <div className="flex-1">
                      <div className="flex justify-between items-baseline mb-1">
                        <h4 className="text-xs font-bold text-brand-text-primary">{n.title}</h4>
        ...
        {notifications.length === 0 && (
          <div className="text-center py-12 bg-gradient-to-br from-brand-card/40 to-brand-secondary/40 rounded-2xl border border-dashed border-brand-text-primary/15 p-6 shadow-md">
            ...
            <p className="text-sm text-brand-text-primary font-bold">Timeline is perfectly clear</p>
