...
export const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  const pages = [
    {
      ...
      illustration: (
        <div className="relative w-full h-64 flex items-center justify-center">
          ...
          {/* Floating News Cards */}
          <div className="absolute top-4 left-6 w-52 p-4 rounded-2xl glass-panel border border-brand-accent/10 shadow-lg"
          >
      ...
      illustration: (
        <div className="relative w-full h-64 flex items-center justify-center">
          ...
          >
            ...
            <div className="flex justify-between items-center mt-2 text-[9px] font-mono text-brand-text-secondary">
              <span>1W Return</span>
              <span className="text-brand-chart-green font-bold">+5.25%</span>
            </div>
          </div>
        </div>
      )
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <div className="text-left">
                <span className="inline-block bg-brand-card text-brand-accent text-[10px] px-3 py-1.5 rounded-2xl rounded-tl-none border border-brand-accent/10">
  ...
  return (
    <div className="fixed inset-0 z-40 bg-gradient-to-b from-brand-primary via-brand-secondary to-brand-primary flex flex-col justify-between p-6 overflow-hidden">
      ...
      <div className="flex flex-col items-center gap-6 pb-6 relative z-10 max-w-md mx-auto w-full">
          ...
          {pages.map((_, idx) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                currentPage === idx ? "w-6 bg-brand-accent" : "w-2 bg-brand-card"
              }`}
            />
          ))}
        </div>
        ...
        <button
          onClick={handleNext}
          className="w-full py-4 rounded-2xl bg-brand-btn-accent hover:bg-brand-accent text-brand-primary font-bold text-sm tracking-wider flex items-center justify-center gap-2 transition-all duration-300 hover:shadow-[0_0_30px_rgba(182,255,90,0.3)] hover:scale-[1.02]"
